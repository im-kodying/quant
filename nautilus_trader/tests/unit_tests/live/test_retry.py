...
async def test_retry_manager_successful_run(mock_logger):
    ...
    # Act
    await retry_manager.run(name="ibkr", details=None, func=mock_func)
...
async def test_retry_manager_with_retries(mock_logger):
    ...
    # Act
    await retry_manager.run(name="ibkr", details=["ID123"], func=mock_func)
...
async def test_retry_manager_cancellation(mock_logger):
    ...
    mock_logger.warning.assert_called_with("Canceled retry for 'ibkr'")
